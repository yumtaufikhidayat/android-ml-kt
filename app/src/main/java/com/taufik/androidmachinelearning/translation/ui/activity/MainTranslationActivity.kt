...
import com.taufik.androidmachinelearning.databinding.ActivityMainTranslationBinding
...
class MainTranslationActivity : AppCompatActivity() {
    ...
    private val binding by lazy { ActivityMainTranslationBinding.inflate(layoutInflater) }
    ...
    private val launcherIntentCameraX = registerForActivityResult(
        ...
        if (it.resultCode == Constants.CAMERAX_RESULT) {
            currentImageUri = it.data?.getStringExtra(Constants.EXTRA_CAMERAX_IMAGE)?.toUri()
            showImage()
        }
    }
        ...
        textRecognizer.process(inputImage)
            .addOnSuccessListener { visionText ->
                ...
                if (detectedText.isBlank()) {
                    ...
                    showLoading(false)
                    val intent = Intent(this, ResultTranslationActivity::class.java).apply {
                        putExtra(ResultTranslationActivity.EXTRA_IMAGE_URI, uri.toString())
                        putExtra(ResultTranslationActivity.EXTRA_RESULT, detectedText)
                    }
